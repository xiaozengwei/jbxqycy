...
import com.gx.core.util.GenericsUtils;
...
public class HibernateEntityDao<T> extends HibernatePagingDao {
	 ...
	 * @return
	 */
	public int getMaxOrderNum(Class<T> className, String maxProperty) {
		Criteria c = super.getSession().createCriteria(className);
		if (maxProperty == null || maxProperty.equals("")) {
			maxProperty = "orderNum";// default orderNum
		}
		c.setProjection(Projections.projectionList().add(
				Projections.max(maxProperty)));
		int max = 0;
		if (c.uniqueResult() != null) {
			try {
				max = (Integer) c.uniqueResult();
			} catch (Exception e) {
				e.printStackTrace();
			}
		}
	 ...
	 * @return
	 */
	public List<T> find(String orderName, boolean isAsc,
			List<PropertyFilter> propertyFilters) {
		Criterion[] c = HibernateUtils.buildCriterion(propertyFilters);
		return find(this.entityClass, orderName, isAsc, c);
	}

	/**
	 * 含有多个order的查询
	 * 
	 * @param orderValue排序键值对
	 * @param propertyFilters
	 * @return
	 */
	public List<T> find(Map<String, String> orderValue,
			List<PropertyFilter> propertyFilters) {
		Criterion[] c = HibernateUtils.buildCriterion(propertyFilters);
		return find(this.entityClass, orderValue, c);
	}
	...
	public Page pagedQuery(String hql, Page page,
			List<PropertyFilter> propertyFilters) {
		return pagedQuery(hql, page.getPageCurrent(), page.getPageSize(),
				propertyFilters);
	}
	 ...
	 * @return
	 */
	public Page pagedQuery1(String hql, Page page, List<PropertyFilter> propertyFilters) {
		return pagedQuery(hql, page.getPageCurrent(), page.getPageSize() ,page.getOrderBy() ,page.getOrder(), propertyFilters);
	}
	 ...
	 * @return
	 */
	public Page pagedQuery(String hql, int pageNo, int pageSize,String orderBy, String order, List<PropertyFilter> propertyFilters) {
		StringBuilder buff = new StringBuilder(hql);
		Map<String, Object> map = new HashMap<String, Object>();
		...
		buff.append(" order by "+orderBy+" "+order);
		return pagedQuery(buff.toString(), pageNo, pageSize, map);
	}
