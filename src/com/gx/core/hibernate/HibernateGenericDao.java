...
import com.gx.core.util.BeanUtils;
import com.gx.core.util.ReflectUtils;
...
public class HibernateGenericDao extends HibernateBasicDao {
	...
	public <T> Criteria createCriteria(Class<T> entityClass,
		...
		Set<String> ordersBy = orderValue.keySet();
		if (ordersBy != null && ordersBy.size() > 0) {

			Criteria criteria = createCriteria(entityClass, criterions);
			for (String orderBy : ordersBy) {
				if (StringUtils.hasText(orderBy)) {
					String isAsc = orderValue.get(orderBy);
					if (StringUtils.hasText(isAsc)&& isAsc.equalsIgnoreCase("asc")) {
						criteria.addOrder(Order.asc(orderBy));
					} else {
						criteria.addOrder(Order.desc(orderBy));
					}
	...
	public <T> List<T> find(Class<T> entityClass, String orderBy,
		...
		return this.createCriteria(entityClass, orderBy, isAsc, criterions)
				.list();
	}
	...
	public <T> List<T> find(Class<T> entityClass, 
			Map<String, String> ordersValue, Criterion... criterions) {

		return this.createCriteria(entityClass, ordersValue, criterions)
				.list();
	}
