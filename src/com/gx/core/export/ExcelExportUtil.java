...
//            XSSFWorkbook xssfWorkbook = new XSSFWorkbook();
            ...
            for (int i = 1; i <= size; i++) {
                ...
                for (int j = 0; j < dList.size(); j++) {
                    ...
                    if(j==0&&dList.get(j).length()==32){
                        cell.setCellValue(i);
                    }
                    if(j==0&&dList.get(j).equals("")){
                        cell.setCellValue(i);
                    }
                }
            }
...
//            XSSFWorkbook xssfWorkbook = new XSSFWorkbook();
            ...
            for (int i = 1; i <= size; i++) {
                ...
                for (int j = 0; j < dList.size(); j++) {
                    ...
                    cell.setCellValue(dList.get(j));
                    if(j==4){
                        cell.setCellValue(new DecimalFormat("######0.00").format(Double.parseDouble(dList.get(3))-Double.parseDouble(dList.get(2))));
                    }
                    if(j==5){
                        cell.setCellValue(new DecimalFormat("######0.00").format(Double.parseDouble(dList.get(4))*0.7364));
                    }
                }
            }
            response.setContentType("application/vnd.ms-excel");
...
//            wb.dispose();
     ...
     * @param metaMap  元信息 clazz类对象,columnNames字段名String[],columnWidth列宽Integer[],columnComment列名String[]
     */
    public void exportBean2(HttpServletResponse response, String fileName, List beanList, Map<String, Object> metaMap) throws Exception {
        try {
...
//            XSSFWorkbook xssfWorkbook = new XSSFWorkbook();
            ...
            for (int i = 1; i <= size; i++) {
                ...
                for (int j = 0; j < dList.size(); j++) {
                    ...
                    cell.setCellValue(dList.get(j));
                    if(j==1){
                        cell.setCellValue(dList.get(4));
                    }
                    if(j==4){
                        cell.setCellValue(new DecimalFormat("######0.00").format(Double.parseDouble(dList.get(3))-Double.parseDouble(dList.get(2))));
                    }
                    if(j==5){
                        cell.setCellValue(new DecimalFormat("######0.00").format((Double.parseDouble(dList.get(3))-Double.parseDouble(dList.get(2)))*0.7364));
                    }
                }
            }
            response.setContentType("application/vnd.ms-excel");
