...
<%
	List<GxSysOrg> orgss = (List<GxSysOrg>)request.getAttribute("orgs5");
%>
...
//ztree列表
		function userlist(event, treeId, treeNode){
			...
			$(event.target).bjuiajax('doLoad', {url:'../hr/index-list2.do', target:treeNode.divid,data:'v_row_Id='+treeNode.id});
			event.preventDefault();
		}
...
<div class="bjui-pageContent">
	<div style="float:left;">
		...
		<div
			style="z-index:1;position:relative;float:left; width:250px; height:620px; margin-bottom:0px; overflow:auto; margin-top:26px; margin-left:0px; border-right:1px #c3ced5 solid; border-bottom:1px #c3ced5 solid;border-left:1px #c3ced5 solid">
			<ul id="ztree3" class="ztree" data-toggle="ztree"
				data-on-click="userlist" data-expand-all="true">
			<%-- 	<%
							for (int i = 0; i < orgs.size(); i++) {
								if ("root".equals(orgs.get(i).getParentOrgId())) {
						%>
				<li data-id="<%=orgs.get(i).getRowId()%>" data-pid="0"
					data-divid="#layout-01" data-tabid="<%=orgs.get(i).getRowId()%>"
					data-faicon="users"><%=orgs.get(i).getOrgName()%></li>
				<%
				...
				<li data-id="<%=orgs.get(j).getRowId()%>"
					...
					data-divid="#layout-01" data-tabid="<%=orgs.get(j).getRowId()%>"
					data-faicon="user"><%=orgs.get(j).getOrgName()%></li>
				<%}}
						%>
				<%}}
						%> --%>

				<c:forEach items="${orgs5}" var="org">
					<c:if test="${org.parentOrgId eq 'root'}">
							<li data-id="${org.rowId}" data-pid="0"
							data-divid="#layout-012" data-tabid="${org.rowId}"
							data-faicon="users">${org.orgName}</li>
							<c:forEach items="${orgs5}" var="org1">
								<c:if test="${org1.parentOrgId ne 'root'&&org.rowId==org1.parentOrgId }">
									<li data-id="${org1.rowId}"
									data-pid="${org1.parentOrgId}"
									data-divid="#layout-012" data-tabid="117${org1.rowId}"
	...
	<div style="margin-left:255px; height:100%;border:1px #c3ced5 solid;"  id="layout-012"></div>
