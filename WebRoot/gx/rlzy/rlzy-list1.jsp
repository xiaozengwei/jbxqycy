...
//ztree列表
		function userlist1(event, treeId, treeNode){
			...
			$(event.target).bjuiajax('doLoad', {url:'<%=basePath%>hr/index-list1.do', target:'#layout-011',data:'v_row_Id='+treeNode.id});
			event.preventDefault();
		}
...
<div class="bjui-pageContent">
	<div style="float:left;">
		...
		<div
			...
			<ul id="ztree2" class="ztree" data-toggle="ztree"
				data-on-click="userlist1" data-expand-all="true" data-options="{nodes:'returnRL2TreeJSON',
                        expandAll: true ,onClick:'userlist1'}">
				<%--<c:forEach items="${orgs_1}" var="org">
					<c:if test="${org.parentOrgId eq 'root'}">
							<li data-id="${org.rowId}" data-pid="0"
							data-divid="#layout-011" data-tabid="${org.rowId}"
							data-faicon="users">${org.orgName}</li>
							<c:forEach items="${orgs_1}" var="org1">
								<c:if test="${org1.parentOrgId ne 'root'&&org.rowId==org1.parentOrgId }">
									<li data-id="${org1.rowId}"
									data-pid="${org1.parentOrgId}"
									data-divid="#layout-011" data-tabid="${org1.rowId+2000}"
	...
	<div style="margin-left:255px; height:100%;border:1px #c3ced5 solid;"  id="layout-011"></div>
