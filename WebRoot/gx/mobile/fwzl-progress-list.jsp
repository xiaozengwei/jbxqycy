...
<header>
    <div class="div-title">
        ...
        <span class="span-title">房屋租赁进度查询</span>
    </div>
</header>
<div class="weui-cells" style="font-size: 15px;">
    <c:choose>
        <c:when test="${fn:length(list) > 0}">
            <c:forEach items="${list }" var="VFwzlProcinst">
                <a class="weui-cell weui-cell_access" style="background-color: white; margin: 0.2em; border-radius: 10px;" href="<%=basePath %>mobile/to-fwzl-view.do?rowId=${VFwzlProcinst.rowId}">
                    <div class="weui-cell__bd">
                        <h4>${VFwzlProcinst.apartmentType}</h4>
                        <p>租赁期限：${VFwzlProcinst.leaseTerm}&nbsp;&nbsp;数量：${VFwzlProcinst.apartmentNumber}</p>
                    </div>
                    <div class="weui-cell__ft">${VFwzlProcinst.applyDate}</div>
                </a>
            </c:forEach>
        </c:when>
        <c:otherwise>
