...
<header>
    <div class="div-title">
        <a href="<%=basePath %>mobile/sd-recharge-select.do" class="a-title"><img src="<%=basePath %>gx/mobile/images/arrow.png" width="25px" align="left" style="margin-top: 10px;"></a>
        <span class="span-title">${title}</span>
    </div>
</header>
<div style="margin-top: 20px;">
    <form action="" method="post" id="dfForm">
        <input type="hidden" value="${rechargeType}" name="rechargeType">
        <div class="weui-cells weui-cells_form">
            ...
            <div class="weui-cell">
                ...
                <div class="weui-cell__bd">
                    <input class="weui-input paddingLeft10" id="rechargeMoney" name="rechargeMoney" type="text" placeholder="请输入充值金额">
                </div>
            </div>
        </div>
    </form>
...
<script>
    ...
    $("#dfPay").click(function() {
        ...
        var zz_rechargeMoney = /^\d+\.?\d{0,2}$/;
        if(!zz_rechargeMoney.test(rechargeMoney)){
            $.toptip('充值金额最多输入两位小数', 'warning');
            return;
        }else{
            if(rechargeMoney == '0.01'){
                $.toptip('充值金额要大于0.01元', 'warning');
                return;
            }
        }
        $.ajax({
            url : '<%=path %>/mobile/sd-recharge.do',
            type : 'post',
