...
<header>
    <div class="div-title">
        ...
        <span class="span-title">会议室租赁进度查询</span>
    </div>
</header>
<div class="weui-cells" style="font-size: 15px;">
    <c:choose>
        <c:when test="${fn:length(list) > 0}">
            <c:forEach items="${list }" var="VMeetProcinst">
                <a class="weui-cell weui-cell_access" style="background-color: white; margin: 0.2em; border-radius: 10px;" href="<%=basePath %>mobile/to-hys-view.do?rowId=${VMeetProcinst.rowId}">
                    <div class="weui-cell__bd">
                        <h4>${VMeetProcinst.selectedRoom}</h4>
                        <p><fmt:formatDate value="${VMeetProcinst.meetStartTime}" pattern="yyyy-MM-dd HH:mm" />~<fmt:formatDate value="${VMeetProcinst.meetEndTime}" pattern="yyyy-MM-dd HH:mm" /></p>
                    </div>
