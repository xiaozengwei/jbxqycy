...
<div class="bjui-pageHeader">
	<form id="pagerForm" data-toggle="ajaxsearch" action="<%=basePath %>fwzl/fwzl-task-list.do" method="post">
		...
		<div class="bjui-searchBar" style="line-height:60px">
			...
			<select name="filter_EQS_apartmentType" data-toggle="selectpicker" style="font-size:15px">
				...
				<option value="标准间" ${param.filter_EQS_apartmentType eq '安装' ? 'selected="selected"':'' }>标准间</option>
				<option value="两室一厅" ${param.filter_EQS_apartmentType eq '改造' ? 'selected="selected"':'' }>两室一厅</option>
			</select>
			&nbsp;
			<label>申请日期：</label>
...
<div class="bjui-pageContent tableContent">
	<table class="table table-bordered table-hover table-striped table-top" data-selected-multi="true">
		...
		<tbody>
		<c:forEach var="vFwzlInstAct" items="${list }" varStatus="status">
			<tr data-id="${vFwzlInstAct.rowId }">
				<td align="center"><input type="checkbox" name="ids" data-toggle="icheck" value="${vFwzlInstAct.rowId}"></td>
				<td align="center">${status.count}</td>
				<td align="center">${vFwzlInstAct.enterpriseName}</td>
				<td align="center">${vFwzlInstAct.enterpriseManager}</td>
				<td align="center">${vFwzlInstAct.contactNumber}</td>
				<td align="center">${vFwzlInstAct.apartmentType }</td>
				<td align="center">${vFwzlInstAct.apartmentNumber }</td>
				<td align="center">${vFwzlInstAct.createUserName }</td>
				<td align="center"><fmt:formatDate value="${vFwzlInstAct.applyDate }" pattern="yyyy-MM-dd" /></td>
				<td align="center">
					<a class="btn btn-green" title="办理" href="<%=basePath%>fwzl/fwzl-handle-page.do?rowId=${vFwzlInstAct.rowId}" data-toggle="navtab" data-id="${vFwzlInstAct.rowId}">办理</a>
				</td>
			</tr>
		</c:forEach>
		</tbody>
